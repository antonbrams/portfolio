...
export const about = [
	...
	},
	{
		...
		hidden: true,
		items: [
			`• Custom <span class='badge'>Agent</span> / <span class='badge'>Memory</span> / <span class='badge'>Tooling</span> Architectures`,
			`• Speech to Text & Text to Speech <span class='badge'>Whisper</span> <span class='badge'>Piper</span>`,
			`• Agent Frameworks <span class='badge'>Deep Agents</span> <span class='badge'>LangGraph</span> <span class='badge'>Langchain</span>`,
			`• Huggingface`,
			`• RAG Systems`,
...
export const projects = [
	{
		info: {
			name: 'Ava',
			description: 'Production AI agent I live inside daily',
			thumbnail: '0.jpg',
		},
		metadata: {
			date: '2025',
		...
		sections: [
			...
			},
			{
				...
				description:
					'The agent controls real services — email, calendar, browser, GitHub, trading, weather. Instead of hardcoding each, I built a Universal Service Registry that normalizes all tool sources (local, MCP, API, peripherals) behind one interface. My Mac connects via inverse SSE as a self-describing peripheral — it announces its tools and schemas on connect. The agent discovers what exists at runtime. Most agent frameworks use "skills" — the LLM reads a tutorial, runs terminal commands, manages API keys, and figures out the integration at runtime. Wasteful, slow, insecure. I replaced this with YAML configs that pre-digest every integration into the most efficient form an LLM can consume — a native tool call with typed arguments. No tutorials, no shell access, no key exposure. API keys are injected from a secure enclave at execution time and sanitized on output — the LLM never sees them. The agent can even author new integrations on the fly — writing YAML that becomes a callable tool — without ever touching credentials.',
			},
			{
				file: '5.jpg',
